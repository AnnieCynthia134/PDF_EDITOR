  ...
  for (const page of doc.getPages()) {
    ...
    for (const [k] of fonts.entries()) {
      ...
      console.log(
        ...
        k.asString(),
        "| Subtype asString:",
        JSON.stringify(subtype?.asString()),
        "| decodeText:",
        JSON.stringify(subtype?.decodeText()),
      );
      ...
      if (descriptor) {
        ...
        console.log(
          "  FontFile2 instanceof PDFRawStream:",
          ff2 instanceof PDFRawStream,
          "| ctor:",
          ff2?.constructor?.name,
        );
